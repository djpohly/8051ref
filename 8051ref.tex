\documentclass[9pt,notitlepage]{extarticle}

\usepackage{compactref}
\usepackage{multicol}
\usepackage{graphicx}
\usepackage{booktabs}
\usepackage{microtype}
\usepackage{tabularx}
\usepackage{multirow}

\newcolumntype{C}{>{\centering\arraybackslash}X}

\title{8051 Quickref}
\author{Willem van Engen}

\begin{document}

\maketitle

\begin{multicols}{3}
  \begin{tabbing}
    \texttt{imm16}		\= = 16bit immediate \\
    \texttt{imm8}		\> =  8bit immediate \\
    \texttt{byte}		\> =  8bit IData addr \\
    \texttt{bit}			\> =  8bit IData bit addr \\
    \texttt{rel}			\> =  8bit Code reladdr signed \\
    \texttt{addr16}		\> = 16bit Code addr \\
    \texttt{addr11}		\> = 11bit Code absaddr \\
    \texttt{i}: 0--1 \hspace{0.15cm} \texttt{n}: 0--7 \hspace{0.15cm}
    \texttt{a}: 20*(absaddr shr 8)
  \end{tabbing}

  \texttt{PSW%
    \addtolength{\columnwidth}{-36pt}%
    \begin{tabularx}{\columnwidth}{@{}C@{}C@{}C@{}C@{}C@{}C@{}C@{}C@{}}
       7 &6 &5 & 4 &3 &2 &1 &0 \\
       C &A &F0 &RS1 &RS0 &V &F1 &P \\
  \end{tabularx} }\\
   \texttt{RS0 RS1}: register bank selection; byte: \\
  \textbf{00}:00-07 \hspace{0.15cm} \textbf{01}:08-0f \hspace{0.15cm}
  \textbf{10}:10-17 \hspace{0.15cm} \textbf{11}:18-1f
\end{multicols}

\ttfamily
\raggedright
\newsavebox\lefttable
\newcommand\A{\texttt{A}}
\newcommand\C{\texttt{C}}
\newcommand\AO[1]{#1\rlap{\normalfont\footnotemark[2]}}
\renewcommand{\thefootnote}{\fnsymbol{footnote}}
\footnotetext[1]{Immediate, direct, and bit addressing require one additional
byte}
\footnotetext[2]{Uses \texttt{A} rather than an immediate value; only this form
affects the \texttt{P} flag}
\footnotetext[3]{\C variant may affect \C flag; \A variant may affect \texttt{P}
flag}
\sbox\lefttable{%
  \begin{tabular}[t]{l>{\normalfont}lcccc>{\normalfont}cc@{\,\,}c@{\,\,}c@{\,\,}c>{\normalfont}c}
  \multicolumn{12}{c}{\small\sffamily\bfseries Arithmetic/byte manipulation}\\
  \toprule
  & Operands & \normalfont \#\rlap{\footnotemark[1]} & \normalfont M\rlap{\footnotemark[1]} & \normalfont @Ri & \normalfont Rn &
    Len & \multicolumn{4}{c}{\normalfont Fl} & Cyc \\
  \midrule
  INC  & op           & \AO{04}& 05 & 06+i & 08+n & 1/2 & & & &\AO{P}& 1 \\
  DEC  & op           & \AO{14}& 15 & 16+i & 18+n & 1/2 & & & &\AO{P}& 1 \\
  ADD  & \A, op           & 24 & 25 & 26+i & 28+n & 1/2 &C&A&V&P& 1 \\
  ADDC & \A, op           & 34 & 35 & 36+i & 38+n & 1/2 &C&A&V&P& 1 \\
  SUBB & \A, op           & 94 & 95 & 96+i & 98+n & 1/2 &C&A&V&P& 1 \\
  DIV  & \texttt{AB}      & 84 &    &      &      & 1 &C& &V&P& 4 \\
  MUL  & \texttt{AB}      & A4 &    &      &      & 1 &C& &V&P& 4 \\
  ORL  & byte, \A         & 42 &    &      &      & 2 & & & & & 1 \\
       & byte, \#imm8     & 43 &    &      &      & 3 & & & & & 2 \\
       & \A, op           & 44 & 45 & 46+i & 48+n & 1/2 & & & &P& 1 \\
       & \C, op           &    & 72 &      &      & 2 &C& & & & 2 \\
       & \C, /op          &    & A0 &      &      & 2 &C& & & & 2 \\
  ANL  & byte, \A         & 52 &    &      &      & 2 & & & & & 1 \\
       & byte, \#imm8     & 53 &    &      &      & 3 & & & & & 2 \\
       & \A, op           & 54 & 55 & 56+i & 58+n & 1/2 & & & &P& 1 \\
       & \C, op           &    & 82 &      &      & 2 &C& & & & 2 \\
       & \C, /op          &    & B0 &      &      & 2 &C& & & & 2 \\
  XRL  & byte, \A         & 62 &    &      &      & 2 & & & & & 1 \\
       & byte, \#imm8     & 63 &    &      &      & 3 & & & & & 2 \\
       & \A, op           & 64 & 65 & 66+i & 68+n & 1/2 & & & &P& 1 \\
  MOV  & op, \#imm8   & \AO{74}&    & 76+i & 78+n & 2/3 & & & &\AO{P} & 1 \\
       & byte, op         & 75 & 85 & 86+i & 88+n & 2/3 & & & & & 2 \\
       & op, byte         &    &    & A6+i & A8+n & 2/3 & & & & & 2 \\
       & \A, op           &    & E5 & E6+i & E8+n & 1/2 & & & &P& 1 \\
       & op, \A           &    & F5 & F6+i & F8+n & 1/2 & & & & & 1 \\
       & bit, \C          &    & 92 &      &      & 2 & & & & & 2 \\
       & \C, bit          &    & A2 &      &      & 2 &C& & & & 2 \\
  SWAP & \A               & C4 &    &      &      & 1 & & & & & 1 \\
  XCH  & \A, op           &    & C5 & C6+i & C8+n & 1/2 & & & &P& 1 \\
  XCHD & \A, op           &    &    & D6+i &      & 1 & & & &P& 1 \\
  DA   & \A               & D4 &    &      &      & 1 &C& &V&P& 1 \\
  DJNZ & op, rel          &    & D5 &      & D8+n & 2/3 & & & & & 2 \\
  CJNE & \A, op, rel      & B4 & B5 &      &      & 3 &C& & & & 2 \\
       & op, \#imm8, rel  &    &    & B6+i & B8+n & 3 &C& & & & 2 \\
  \bottomrule
\end{tabular}
}

\newlength\lefttableheight
\settoheight\lefttableheight{\usebox\lefttable}
\newlength\lefttabledepth
\settodepth\lefttabledepth{\usebox\lefttable}
\addtolength\lefttableheight\lefttabledepth
\newlength\lefttablewidth
\settowidth\lefttablewidth{\usebox\lefttable}
\newlength\righttablewidth
\setlength\righttablewidth\textwidth
\addtolength\righttablewidth{-\lefttablewidth}

\begin{minipage}[t]{\lefttablewidth}\usebox\lefttable\end{minipage}%
  \begin{minipage}[t][\lefttableheight][t]{\righttablewidth}
  \centering
    \begin{tabular}[t]{lcc>{\normalfont}l>{\normalfont}c>{\normalfont}c}
      \multicolumn{6}{c}{\small\sffamily\bfseries Control} \\
  \toprule
  & \normalfont Op & \normalfont Op2 & \normalfont Operands &
    \normalfont Len & \normalfont Cyc \\
  \midrule
  NOP  & 00 &    &          & 1 & 1 \\
  JBC  & 10 &    & bit, rel & 3 & 2 \\
  \normalfont \texttt{JB}/\texttt{JNB}
       & 20 & 30 & bit, rel & 3 & 2 \\
  \normalfont \texttt{JC}/\texttt{JNC}
       & 40 & 50 & rel      & 2 & 2 \\
  \normalfont \texttt{JZ}/\texttt{JNZ}
       & 60 & 70 & rel      & 2 & 2 \\
  SJMP & 80 &    & rel      & 2 & 2 \\
  \normalfont \texttt{PUSH}/\texttt{POP}
       & C0 & D0 & byte     & 2 & 2 \\
  \normalfont \texttt{AJMP}/\texttt{ACALL}
       & 01+a & 11+a & addr11 & 2 & 2 \\
  \normalfont \texttt{LJMP}/\texttt{LCALL}
       & 02 & 12 & addr16   & 3 & 2 \\
  \normalfont \texttt{RET}/\texttt{RETI}
       & 22 & 32 &          & 1 & 2 \\
  \bottomrule
\end{tabular}

\vfill
% bit requires one extra byte, C may affect C, A may affect P
\begin{tabular}{lccc>{\normalfont}cc>{\normalfont}c}
  \multicolumn{7}{c}{\small\sffamily\bfseries Bit manipulation} \\
  \toprule
  & \normalfont bit\rlap{\footnotemark[1]} & \C & \A & Len & Fl & Cyc \\
  \midrule
  RR   &    &    & 03 & 1 &       & 1 \\
  RRC  &    &    & 13 & 1 &C\,\,P & 1 \\
  RL   &    &    & 23 & 1 &       & 1 \\
  RLC  &    &    & 33 & 1 &C\,\,P & 1 \\
  CPL  & B2 & B3 & F4 & 1/2 & (C\,\,P)\rlap{\footnotemark[3]} & 1 \\
  CLR  & C2 & C3 & E4 & 1/2 & (C\,\,P)\rlap{\footnotemark[3]} & 1 \\
  SETB & D2 & D3 &    & 1/2 & (C\,\,P)\rlap{\footnotemark[3]} & 1 \\
  \bottomrule
\end{tabular}

\vfill
\begin{tabular}{llc>{\normalfont}cc>{\normalfont}c}
  \multicolumn{6}{c}{\small\sffamily\bfseries Code/XData segments} \\
  \toprule
  & \normalfont Operands & \normalfont Op & \normalfont Len &
    \normalfont Fl & \normalfont Cyc \\
  \midrule
  MOVX & \A, @\texttt{DPTR}     & E0   & 1 & P & 2 \\
       & \A, @Ri                & E2+i & 1 & P & 2 \\
       & @\texttt{DPTR}, \A     & F0   & 1 &   & 2 \\
       & @Ri, \A                & F2+i & 1 &   & 2 \\
  JMP  & @\A+\texttt{DPTR}      & 73   & 1 &   & 2 \\
  MOVC & \A, @\A+\texttt{PC}    & 83   & 1 & P & 2 \\
       & \A, @\A+\texttt{DPTR}  & 93   & 1 & P & 2 \\
  MOV  & \texttt{DPTR}, \#imm16 & 90   & 3 &   & 2 \\
  INC  & \texttt{DPTR}          & A3   & 1 &   & 2 \\
  \bottomrule
\end{tabular}
\end{minipage}

\newpage

\section*{Memory organization}
\includegraphics[width=10cm]{8051mem.png}

\section*{SFRs}


\end{document}
